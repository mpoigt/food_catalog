...
- Пересчёт цены в USD по курсу Нацбанка РБ: курс запрашивается раз в день и кэшируется
  (на сервере — в Redis, на клиенте — на текущую дату), цены считаются из одного курса.
